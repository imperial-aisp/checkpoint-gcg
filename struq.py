...
def format_with_other_delimiters(text, test=False):
    ...
    mark = (
        np.random.choice(
            OTHER_DELM_TOKENS["mark"][test_idx:]
            if test
            else OTHER_DELM_TOKENS["mark"][:test_idx]
        )
        + ":"
    )
        ...
        if data_dicts[i].get("input", "") == "":
            ...
            if attack == "Naive":
                ...
                data_dicts_item["input"] += (
                    ...
                    + "\n"
                    + ref_inst_resp.get(
                        data_dicts_item["instruction"], data_dicts_item["output"]
                    )
                    + "\n\n"
