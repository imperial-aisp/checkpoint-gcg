...
from gcg.utils import test_model_output_prompt_injection
...
class AttackBuffer:
    ...
    def log_buffer(self, tokenizer):
        ...
        for loss, ids in self.buffer:
            optim_str = tokenizer.batch_decode(ids, skip_special_tokens=True,
...
def filter_ids(ids: Tensor, tokenizer: transformers.PreTrainedTokenizer):
    ...
    """
    ids_decoded = tokenizer.batch_decode(ids, skip_special_tokens=True,
...
def tensor_to_json(obj):
    if isinstance(obj, torch.Tensor):
        ...
        return [tensor_to_json(v) for v in obj]
    else:
        return obj
...
class GCG:
    ...
    def apply_fschat_conv_template(self, conv_template: Conversation, message: str):
        conv_template.messages = []
        conv_template.append_message(conv_template.roles[0], message)
        sep = copy.deepcopy(conv_template.sep)
        conv_template.sep = ""
        prompt = conv_template.get_prompt()
        conv_template.sep = sep
        return prompt + conv_template.sep + conv_template.roles[1]+ "\n"
    ...
    def _test_suffix(
        ...
        in_response, begin_with, outputs = test_model_output_prompt_injection(
                [template.replace("{optim_str}", adv_suffix)],
                target,
                self.model,
                self.tokenizer,
                self.config.seq_len,
                is_sep_dataset=self.config.is_sep_dataset,
            )

        return in_response, begin_with, outputs[0][0]
    ...
    def run(
        ...
        template = self.apply_fschat_conv_template(conv_template, messages)
        ...
        for i in tqdm(range(config.num_steps)):
            # set n_replace = 3 to the first 1/3 of the steps; 2 to the second 1/3; 1 to the last 1/3
            if i < config.num_steps // 3:
                config.n_replace = 3
            elif i < 2 * config.num_steps // 3:
                config.n_replace = 2
            else:
                config.n_replace = 1
            ...
            with torch.no_grad():
                ...
                if self.config.probe_sampling_config is None:
                    loss, first_match_idx = find_executable_batch_size(self._compute_candidates_loss_original, batch_size)(input_embeds)
            ...
            optim_str = tokenizer.batch_decode(optim_ids, skip_special_tokens=True,
                clean_up_tokenization_spaces=False)[0]
            optim_strings.append(optim_str)
            optim_ids_list.append(tensor_to_json(optim_ids.squeeze(0)))

            log_dict = {
                    "step": i+1, 
                    "loss": f"{current_loss:.4f}",
                    "suffix": optim_str,
                    "optim_ids": tensor_to_json(optim_ids.squeeze(0))
                }
            ...
            begin_with = False
            if self.stop_flag:
                logger.info("Evaluating suffix for early stopping...")
                current_loss_exact_match = loss[first_match_idx].item()
                optim_ids_exact_match = sampled_ids[first_match_idx].unsqueeze(0)
                optim_str_exact_match = tokenizer.batch_decode(optim_ids_exact_match, skip_special_tokens=True,
                clean_up_tokenization_spaces=False)[0]
                in_response, begin_with, output = self._test_suffix(
                    template,
                    optim_str_exact_match,
                    target,
                )
                log_dict = {
                    "step": i+1, 
                    "stop_flag": self.stop_flag,
                    "loss": f"{current_loss_exact_match:.4f}",
                    "suffix": optim_str_exact_match,
                    "optim_ids": tensor_to_json(optim_ids_exact_match.squeeze(0)),
                    "success_begin_with": begin_with,
                    "success_in_response": in_response,
                    "generated_output": output,
                }

            # if i == 0 or (i+1) % 5 == 0 or self.stop_flag:  
            ...
            if begin_with:
                ...
                best_loss = current_loss_exact_match
                best_string = optim_str_exact_match
                best_tokens = optim_ids_exact_match.squeeze(0)
                break
            else:
                self.stop_flag = False
        ...
        if not self.stop_flag:
            ...
            best_tokens = optim_ids_list[min_loss_index]
    ...
    def init_buffer(self) -> AttackBuffer:
        ...
        if isinstance(config.optim_str_init, str):
            ...
            if config.buffer_size > 1:
                ...
                sampled_init_buffer_ids = init_buffer_ids[init_indices]
                
                if config.filter_ids:
                    while True:
                        try:
                            sampled_init_buffer_ids = filter_ids(sampled_init_buffer_ids, tokenizer)
                            break
                        except RuntimeError as e:
                            init_indices = torch.randint(0, init_buffer_ids.shape[0], (config.buffer_size - 1, init_optim_ids.shape[1]))
                            sampled_init_buffer_ids = init_buffer_ids[init_indices]
                    if sampled_init_buffer_ids.shape[0] < config.buffer_size - 1:
                        num_missing = (config.buffer_size - 1) - sampled_init_buffer_ids.shape[0]
                        sampled_init_buffer_ids = torch.cat(
                            [sampled_init_buffer_ids, init_optim_ids.repeat(num_missing, 1)], dim=0
                        )
                        
                init_buffer_ids = torch.cat([init_optim_ids, sampled_init_buffer_ids], dim=0)
            else:
        ...
        init_buffer_losses, first_match_idx = find_executable_batch_size(self._compute_candidates_loss_original, true_buffer_size)(init_buffer_embeds)
    ...
    def _compute_candidates_loss_original(
        ...
        for i in range(0, input_embeds.shape[0], search_batch_size):
            with torch.no_grad():
                ...
                if self.config.early_stop:
                    matches = torch.all(torch.argmax(shift_logits, dim=-1) == shift_labels, dim=-1)
                    if torch.any(matches):
                        first_match_idx = i + torch.nonzero(matches)[0].item()
                        self.stop_flag = True
        ...
        return torch.cat(all_loss, dim=0), first_match_idx
    ...
    def _compute_candidates_loss_probe_sampling(
        ...
        def _compute_probe_losses(result_queue: queue.Queue, search_batch_size: int, probe_embeds: Tensor) -> None:
            probe_losses, first_match_idx = self._compute_candidates_loss_original(search_batch_size, probe_embeds)
        ...
        def _convert_to_draft_tokens(token_ids: Tensor) -> Tensor:
            decoded_text_list = self.tokenizer.batch_decode(token_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)
        ...
        alpha = (1 + rank_correlation) / 2 if not math.isnan(rank_correlation) else 0.5
        ...
        filtered_losses, first_match_idx = self._compute_candidates_loss_original(search_batch_size, filtered_embeds)
...
def run(
    ...
    result = gcg.run(messages, target, conv_template, logfile)
    return result
